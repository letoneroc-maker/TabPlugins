...
async function clearAllArchived() {
  ...
  for (const tab of deferred) {
    if (tab.completed) tab.dismissed = true;
  }
  await chrome.storage.local.set({ deferred });
}
...
function buildOverflowChips(hiddenTabs, urlCounts = {}) {
  const hiddenChips = hiddenTabs.map(tab => {
    ...
    const dupeTag  = count > 1 ? ` <span class="chip-dupe-badge">(${count}次)</span>` : '';
...
async function renderDeferredColumn() {
  ...
  try {
    ...
    console.warn('[tab-out] 无法加载已保存的标签页:', err);
    column.style.display = 'none';
  }
}
...
function renderArchiveItem(item) {
  ...
  return `
    <div class="archive-item">
      ...
      <button class="archive-item-delete" data-action="delete-archived" data-archived-id="${item.id}" title="删除">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12" /></svg>
      </button>
    </div>`;
}
...
async function renderStaticDashboard() {
  ...
  if (domainGroups.length > 0 && openTabsSection) {
    if (openTabsSectionTitle) openTabsSectionTitle.textContent = '正在浏览';
...
document.addEventListener('click', async (e) => {
  ...
  // ---- Delete a single archived tab ----
  if (action === 'delete-archived') {
    const id = actionEl.dataset.archivedId;
    if (!id) return;
...
document.addEventListener('click', (e) => {
  const clearBtn = e.target.closest('#archiveClearBtn');
  if (!clearBtn) return;

  if (confirm('确定要清空所有归档吗？')) {
...
function applyCustomColor(hexColor) {
  ...
  if (luminance > 0.5) {
    root.style.setProperty('--ink', '#1a1613');
    root.style.setProperty('--paper', color);
    root.style.setProperty('--card-bg', color);
    root.style.setProperty('--warm-gray', '#c0b8ae');
    root.style.setProperty('--muted', '#6a6058');
  } else {
    root.style.setProperty('--ink', '#f0ece6');
    root.style.setProperty('--paper', `rgb(${r}, ${g}, ${b})`);
    root.style.setProperty('--card-bg', `rgb(${Math.max(0, r - 20)}, ${Math.max(0, g - 20)}, ${Math.max(0, b - 20)})`);
    root.style.setProperty('--warm-gray', `rgba(255, 255, 255, 0.1)`);
    root.style.setProperty('--muted', `rgba(255, 255, 255, 0.6)`);
  }
  
  root.style.setProperty('--accent-amber', color);
  root.style.setProperty('--accent-amber-rgb', `${r}, ${g}, ${b}`);
  root.style.setProperty('--accent-amber-light', `rgba(${r}, ${g}, ${b}, 0.25)`);
  root.style.setProperty('--accent-amber-border', `rgba(${r}, ${g}, ${b}, 0.6)`);
  
  chrome.storage.local.set({ customThemeColor: color });
  showToast('主题已更新');
}
...
function resetColor() {
  const root = document.documentElement;
  root.style.setProperty('--ink', '#1a1613');
  root.style.setProperty('--paper', '#f8f5f0');
  root.style.setProperty('--card-bg', '#fffdf9');
  root.style.setProperty('--warm-gray', '#e8e2da');
  root.style.setProperty('--muted', '#9a918a');
  root.style.setProperty('--accent-amber', '#c8713a');
  root.style.setProperty('--accent-amber-rgb', '200, 113, 58');
  root.style.setProperty('--accent-amber-light', 'rgba(200, 113, 58, 0.08)');
  root.style.setProperty('--accent-amber-border', 'rgba(200, 113, 58, 0.3)');
  
  chrome.storage.local.remove('customThemeColor');
  showToast('已重置为默认');
}

async function loadSavedColor() {
  try {
    const { customThemeColor } = await chrome.storage.local.get('customThemeColor');
    if (customThemeColor) {
      const root = document.documentElement;
    ...
    console.warn('[tab-out] 加载保存的主题失败:', e);
  }
}

document.getElementById('easterEggTrigger').addEventListener('click', handleEasterEggClick);
...
document.getElementById('applyColor').addEventListener('click', () => {
  ...
  if (input && input.value) {
    applyCustomColor(input.value.trim());
  }
});
